    ...
    with open("input.txt") as file:
        while (line := file.readline()):
            ...
            if monkey_id:
                ...
                m.append(Monkey(int(monkey_id), items, divisible_nr, operation, operation_nr, [monkey1, monkey2]))
...
def main():
    ...
    while round <= 20:
        for m in monkeys:
            while len(m.items) > 0:
                m.inspect_counter += 1
                new_worry_level = calculate(m.operation, m.items.pop(0), m.operation_nr)
                new_worry_level = int(new_worry_level / 3)
                
                divisible = new_worry_level % int(m.divisible_nr) == 0
                
                if divisible == m.monkeys[0][0]:
